...
namespace gpxpy {
     ...
     * trainable
     */
    GP::GP(std::vector<double> input, std::vector<double> output, int n_tiles,
           int n_tile_size, double l, double v, double n, int n_r,
                             ...
                             int iter)
    {
        ...
        hpx::run_as_hpx_thread([this, &loss, &hyperparams, iter]() {
            loss = optimize_step_hpx(_training_input, _training_output,
                                     _n_tiles, _n_tile_size, lengthscale,
                                     vertical_lengthscale, noise_variance,
                                     n_regressors, hyperparams,
                                     trainable_params, iter)
                       .get();  // Wait for and get the result from the future
        });
        return loss;
    }
    ...
    double GP::calculate_loss()
    {
        ...
        hpx::run_as_hpx_thread([this, &loss, &hyperparameters]() {
            loss = compute_loss_hpx(_training_input, _training_output, _n_tiles,
                                    _n_tile_size, n_regressors, hyperparameters)
                       .get();  // Wait for and get the result from the future
        });
        return loss;
    }
