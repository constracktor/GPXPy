...
def rhs(X_train, Y_train, hyperparam):
    K = 1.0*rbf_kernel(X_train, gamma=0.5*hyperparam) + 0.1*np.eye(X_train.shape[0])
...
def objective(X_train, Y_train, L, alpha):
    N = X_train.shape[0]
    loss =  np.sum(np.log(np.diag(L)**2, dtype='f')) + Y_train.T.dot(alpha) + N * np.log(2*np.pi, dtype='f')
    return 0.5*loss/N
...
def derivative(X_train, Y_train, K, alpha, hyperparam):
    N = X_train.shape[0]
    # grad_noise = np.eye(X_train.shape[0], X_train.shape[0]) * np.sqrt(hyperparam) * 2
    # return 0.5 * 1/N * np.trace( np.linalg.solve(K, (np.eye(X_train.shape[0]) - np.outer(Y_train, alpha))) @  grad_noise )
    # grad_v = 2.0 * np.sqrt(hyperparam) * rbf_kernel(X_train, gamma=0.5)
    # return 0.5 * 1/N * np.trace( np.linalg.solve(K, (np.eye(X_train.shape[0]) - np.outer(Y_train, alpha))) @  grad_v )
    grad_l = 1.0 / (sqrt(hyperparam) * hyperparam) * rbf_kernel(X_train, gamma=0.5*hyperparam) * compute_distance_matrix(X_train, X_train)
