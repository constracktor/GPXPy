...
do
  ...
  do
    cd ../build && ./hpx_cholesky --n_train $i --n_test $N_TEST --n_regressors $N_REG --n_tiles $N_TILES --cholesky $CHOLESKY | tee -a $APEX_FILE
    cd ../benchmark_scripts && ./output_formater.sh $N_TILES $i $N_TEST $N_REG $CHOLESKY $APEX_FILE $OUTPUT_FILE
  done
done
